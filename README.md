...
- **REFMOD REFERENCES (v0.4.41):** REF2VA adds a **REFMOD** button immediately after **INPUT SCALING**. It opens a separate explanatory overlay—without expanding the node—to select standalone saved H3 person references from `ComfyUI/models/refmods/` and its subfolders. The optional [ComfyUI-MiniMaxH3Mod](https://github.com/Luisacaotica/ComfyUI-MiniMaxH3Mod) pack can create these files, but is not required to read them. The description saved in the workflow row overrides file metadata. This first version rejects multi-member bundle files and applies strength as simple latent scaling; use standalone image/video/audio files.
