...
function install(node) {
  ...
  const refmodTimelineItems = () => {
    ...
    const occupied = { image: new Set(), video: new Set(), audio: new Set() };
    for (const item of activeItems()) occupied[laneForItem(item)].add(item.slot);
    const results = [];
    for (const row of rows) {
      const entry = refModLibrary.entries?.find(e => e.name === row.name);
      if (!entry) continue;
      let laneType = entry.kind === "video" ? "video" : entry.kind === "audio" ? "audio" : "image";
      const maxSlots = laneType === "image" ? MAX.image : laneType === "video" ? MAX.video : MAX.audio;
      let slot = 0;
      while (slot < maxSlots && occupied[laneType].has(slot)) slot++;
      if (slot >= maxSlots) continue;
      occupied[laneType].add(slot);
      results.push({ id: `refmod-${row.slot}`, type: laneType, value: row.name, slot, start: slot, duration: laneType === "image" ? 1 : 2, _isRefMod: true, refmodSlot: row.slot, strength: row.strength ?? 1 });
    }
    return results;
  };
  ...
  const render = () => {
    ...
    if (mode() === "REF2VA") {
      ...
      refModActiveBadge = document.createElement("span"); refModActiveBadge.className = "ds-h3-refmod-badge"; refModButton.append(refModLabel, refModActiveBadge); updateRefModActiveBadge();
