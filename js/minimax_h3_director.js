...
function install(node) {
  ...
  function openRefModOverlay(trigger) {
    ...
    const redraw = () => {
      ...
      for (const row of [...rows].sort((a, b) => a.slot - b.slot)) {
        ...
        select.onchange = () => { row.name = select.value; const entry = refModLibrary.entries.find(item => item.name === row.name); if (entry) row.description = entry.description || ""; row.enabled = Boolean(row.name); emit(); redraw(); };
  ...
  const refmodTimelineItems=()=>{return(state.refmods||[]).filter(row=>row.enabled!==false&&row.name&&Number(row.strength)!==0).map((row,i)=>{let mediaType=row.media_type;if(!mediaType){if(refModLibrary.loaded){const entry=refModLibrary.entries.find(e=>e.name===row.name);mediaType=entry?.media_type||"image";}else{mediaType="image";}}let lane="image";if(mediaType==="video")lane="video";else if(mediaType==="audio")lane="audio";return{id:`refmod_${i}`,type:"text",value:`REFMOD ${i+1} (${row.name})`,start:0,duration:state.duration,lane,_isRefMod:true,strength:Number(row.strength)||1,media_type:mediaType}})};;
