...
| FFmpeg preview subprocess | `nodes/nodes_enhanced_video_combine.py:54-112` | Request supplies a basename and subfolder for an output asset; node execution supplies encoded frames and an output path | The required `imageio-ffmpeg` executable from ComfyUI's Python environment is used by default; host `PATH` lookup is enabled only by the explicit `DASIWA_ALLOW_SYSTEM_FFMPEG=1` operator setting. Only `type=output` is accepted; basename/traversal/commonpath checks constrain the preview source to the ComfyUI output root; FFmpeg commands use argument lists, not shells | An FFmpeg vulnerability or a regression in the fixed command builder could affect the server | Retain for manual review | `.tests/test_enhanced_video_combine.py`; executable-resolution and preview-root security tests |
