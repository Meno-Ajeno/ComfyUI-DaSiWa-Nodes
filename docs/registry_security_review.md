...
| FFmpeg preview subprocess | `nodes/nodes_enhanced_video_combine.py:66-111` | Request supplies a basename and subfolder for an output asset; node execution supplies encoded frames and an output path | Only `type=output` is accepted; basename/traversal/commonpath checks constrain the preview source to the ComfyUI output root; FFmpeg commands use argument lists, not shells | An FFmpeg vulnerability or a regression in the fixed command builder could affect the server | Retain for manual review | Existing `.tests/test_enhanced_video_combine.py`; preview-root security test |
