...
Models must already be present on disk before ComfyUI starts. Download Hugging Face models with the official tooling, review their provenance, and place the complete folder in `ComfyUI/models/llm`. Runtime downloads, Hugging Face token reads, and custom remote model code are deliberately unsupported: values supplied through ComfyUI's `/prompt` API must never choose code for the server to download or execute.
