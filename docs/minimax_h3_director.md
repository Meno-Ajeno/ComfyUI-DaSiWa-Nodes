...
Each selected row stores its editable description in the workflow. That workflow description takes precedence over the description embedded in the file. The overlay's **INSERT IN PROMPT** button writes the expanded native Picture, Video, or Audio label at the cursor; `<RefMod N>` aliases remain supported in saved workflows and are translated when queued.
