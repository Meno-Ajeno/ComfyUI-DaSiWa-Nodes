...
- **FFmpeg executable isolation:** The node uses the executable supplied by the required `imageio-ffmpeg` package in ComfyUI's Python environment and does not search the host `PATH` by default. Operators who need codecs or hardware backends absent from that bundled build can explicitly select the system executable by setting `DASIWA_ALLOW_SYSTEM_FFMPEG=1` before starting ComfyUI.
